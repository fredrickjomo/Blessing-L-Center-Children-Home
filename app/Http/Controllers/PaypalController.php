...
class PaypalController extends Controller
{
    ...
    public function expressCheckout(Request $request){
        ...
        paypal responds with with an array of data containing a link
        to paypal's payment system
        */
        $response=$this->provider->setExpressCheckOut($cart,$recurring);

        //redirect back if no link found

        if(!$response['paypal_link']){
            flash('Something went wrong with paypal.try again')->error();
    ...
    public function getCart($recurring,$invoice_id){
if($recurring){
    return[
        'items'=>[
        ...
        //paypal returns url after successful payment
        'return_url'=>url('/paypal/express-checkout-success?recurring=1'),
        'subscription_description'=>'Monthly Subscription'.config('paypal.invoice_prefix').'#'.$invoice_id,
